#!/usr/bin/env node
/**
 * UAT Test Script for @csuwl/opencode-memory-plugin
 * Tests all 12 tools and core features
 */
...
// Run OpenCode tool command
async function runTool(toolName, args) {
  const argsJson = JSON.stringify(args);
  const result = await runCommand('opencode', ['tool', toolName, argsJson], 60000);
  return result;
}
...
async function testMemoryWrite() {
  ...
  // Test basic write
  try {
    const result = await runTool('memory_write', {
...
async function testMemoryRead() {
  ...
  try {
    const result = await runTool('memory_read', { file: 'MEMORY.md' });
    const parsed = JSON.parse(result.stdout);
  ...
  for (const file of coreFiles) {
    try {
      const result = await runTool('memory_read', { file });
      const parsed = JSON.parse(result.stdout);
  ...
  try {
    const result = await runTool('memory_read', { file: 'NONEXISTENT.md' });
    const parsed = JSON.parse(result.stdout);
  ...
  try {
    const result = await runTool('memory_read', { file: '../etc/passwd' });
    const parsed = JSON.parse(result.stdout);
...
async function testMemorySearch() {
  ...
  // Test basic search
  try {
    const result = await runTool('memory_search', { query: 'UAT Test' });
    const parsed = JSON.parse(result.stdout);
  ...
  try {
    const result = await runTool('memory_search', { query: 'personality', file: 'SOUL.md' });
    const parsed = JSON.parse(result.stdout);
...
async function testVectorMemorySearch() {
  log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', 'cyan');
  ...
  try {
    const result = await runTool('vector_memory_search', {
...
async function testDailyLogs() {
  ...
  // Test init_daily
  try {
    const result = await runTool('init_daily', {});
    const parsed = JSON.parse(result.stdout);
  ...
  // Test list_daily
  try {
    const result = await runTool('list_daily', { days: 7 });
    const parsed = JSON.parse(result.stdout);
...
async function testSessions() {
  ...
  try {
    const result = await runTool('save_session', {
  ...
  // Test list_sessions
  try {
    const result = await runTool('list_sessions', { limit: 10 });
    const parsed = JSON.parse(result.stdout);
...
async function testIndexManagement() {
  ...
  // Test index_status
  try {
    const result = await runTool('index_status', {});
    const parsed = JSON.parse(result.stdout);
  ...
  try {
    const result = await runTool('configure_index', {
      autoUpdate: true,
      debounceDelay: 2000,
      batchSize: 5
    });
  ...
  try {
    const result = await runTool('configure_index', { debounceDelay: -100 });
    const parsed = JSON.parse(result.stdout);
  ...
  // Test update_index
  try {
    const result = await runTool('update_index', { force: false });
    const parsed = JSON.parse(result.stdout);
  ...
  // Test rebuild_index
  try {
    const result = await runTool('rebuild_index', { force: false });
    const parsed = JSON.parse(result.stdout);
  ...
  // Check vector database file exists
...
async function testVectorStore() {
  ...
  try {
    const result = await runTool('rebuild_index', { force: true });
    const parsed = JSON.parse(result.stdout);
...
async function testErrorHandling() {
  ...
  // Test memory_write missing content
  try {
    const result = await runTool('memory_write', {});
    const parsed = JSON.parse(result.stdout);
  ...
  try {
    const result = await runTool('configure_index', { batchSize: 0 });
    const parsed = JSON.parse(result.stdout);
